...
const CONSEJOS_DATA = {
  ...
  cultivo: {
    ...
    consejos: [
      { icono:'🏪', titulo:'Semillas en growshop — qué compras',
        texto:'En growshops españoles lo habitual es comprar <strong>semillas feminizadas o autoflorecientes</strong> (pack de 1–5+). Germinas en casa → plántula → hidro. También venden <strong>esquejes enraizados</strong> (clones) o te los regala un conocido; no hace falta semilla si ya tienes genética estable.',
        alerta:{ tipo:'info', txt:'ℹ️ El pack trae la ficha del breeder: tiempos, altura y a veces EC. En la app enlazamos al semillero de referencia por variedad — confirma el tuyo en tienda.' } },
      { icono:'🌸', titulo:'Resultado del cultivo — flores (cogollos)',
        texto:'El <strong>producto final</strong> del ciclo de floración son los <strong>cogollos</strong> (inflorescencias femeninas): resina, terpenos y madurez según tricomas. Tras <strong>cosechar</strong>, secas/curas y vacías el depósito. Eso cierra <em>ese</em> ciclo productivo.',
        alerta:{ tipo:'ok', txt:'✅ La app te guía hasta la ventana de cosecha (EC/pH, HR en flor, calendario). Secado y curado: ver consejos «Secado» y «Curado en frasco» en esta pestaña.' } },
      { icono:'✂️', titulo:'Esquejes — repetir genética sin volver a sembrar',
        texto:'<strong>No es lo mismo que la cosecha.</strong> De una planta en <strong>vegetativo</strong> (o una <strong>madre</strong> 18/6) cortas ramas → enraizan en domo → nuevas plantas <em>idénticas</em> genéticamente. Sirve para SOG, madres DWC o no depender de semillas cada ciclo. Las <strong>autos</strong> no se usan como madre clásica.',
        alerta:{ tipo:'warn', txt:'⚠️ Puedes cosechar flores Y mantener una rama/madre para esquejes — son flujos distintos: flor = 12/12; madre = 18/6 permanente.' } },
      ...
      { icono:'✂️', titulo:'Estado «Cosecha» en Cultivo e instalación',
        texto:'Cuando la app marca <strong>Cosechar</strong> (✂️ / punto rojo) significa que, según la <strong>genética</strong> y los <strong>días desde el trasplante</strong>, estás en la ventana final del ciclo (~85–100 % del tiempo estimado). Es la señal de que <strong>conviene revisar tricomas y preparar el corte de cogollos</strong> — no un timer exacto al minuto.',
        alerta:{ tipo:'warn', txt:'⚠️ Confirma siempre con lupa: tricomas lechosos (más cerebral) → ámbar (más sedante). La fecha de la ficha debe ser el día real de entrada al hidro.' } },
      { icono:'🌸', titulo:'Qué cortar al cosechar',
        texto:'Cortas las <strong>inflorescencias</strong> (cogollos/colas): ramas floridas con resina madura. No confundir con <strong>esquejes</strong> (ramas vegetativas para clonar). Tras cortar: retira hojas grandes, deja cogollo limpio y pasa a <strong>secado</strong>. En hidro: pulsa <strong>Cosechar y registrar</strong> en la ficha de la maceta y planifica vaciado/limpieza del depósito.',
        alerta:{ tipo:'ok', txt:'✅ HidroGrow registra la cosecha en historial; el secado y curado los haces fuera del depósito (sala secado / frascos).' } },
      { icono:'🍃', titulo:'Secado del cogollo',
        texto:'Colgar ramas enteras o usar <strong>red de secado</strong> en cuarto oscuro, <strong>18–20 °C</strong>, <strong>HR 45–55 %</strong>, ventilación suave (no directa sobre cogollos). Típico <strong>7–14 días</strong> hasta que ramita fina <strong>parte al doblar</strong> (no se dobla húmeda). Evita secar demasiado rápido (&gt;24 °C o HR muy baja): pierdes terpenos.',
        alerta:{ tipo:'info', txt:'ℹ️ Growshops venden higrómetros y redes; un armario vacío con extractor bajo y humidificador ayuda a estabilizar HR.' } },
      { icono:'🫙', titulo:'Curado en frasco',
        texto:'Mete cogollos en <strong>tarros de vidrio</strong> llenos ~75 %, tapa hermética. Primeras <strong>2 semanas</strong>: abre (burping) <strong>2–5 min/día</strong> para soltar humedad y CO₂. Objetivo <strong>58–62 % HR</strong> dentro del frasco (bolsa Boveda 62 % opcional). Curado mínimo orientativo <strong>2–4 semanas</strong>; muchos cultivadores prefieren <strong>4–8 semanas</strong> en genéticas resinosas.',
        alerta:{ tipo:'warn', txt:'⚠️ Si huele a heno o hay moho: HR alta al cerrar — saca, seca un poco más y vuelve a curar con burping más frecuente.' } },
      { icono:'🧹', titulo:'Tras cosechar en hidro (DWC/RDWC)',
        texto:'Con el cultivo fuera del cubo: <strong>vacía el depósito</strong>, limpia raíces/biofilm, enjuaga con agua limpia o H₂O₂ diluido según tu protocolo. Prepara <strong>nueva mezcla</strong> solo cuando tengas plántulas/esquejes listos para el siguiente ciclo. El secado/curado ocurre en paralelo, fuera del sistema hidropónico.',
        alerta:{ tipo:'ok', txt:'✅ Checklist de recarga en la app cubre limpieza y primer llenado del siguiente ciclo.' } },
